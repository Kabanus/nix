...
print(intOptional3!) // Явное извлечение значения с помощью знака "!" (значение не должно быть пустым)
```
```
// Неявное извлечение
var str: String! // знак "!" означает "упаковку" в контейнер (по-умолчанию присваиватся значение nil)
print(str) // автоматическое извлечение значения (значения может быть пустым)
```
