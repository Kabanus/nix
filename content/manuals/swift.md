...
1. Извлечение значений вручную, где это необходимо в коде
2. Для переменных, которые часто могут быть nil
3. Для всех вариантов, где не подходит **implicit unwrapping**
* Неявное (implicit unwrapping) - символ "!" при объявлении
1. Для переменных или констант, которые нужно обязательно один раз заполнить (проинициализировать) и потом везде использовать
2. Константы, которые инициализируются при создании объекта
3. Позволяет НЕ выполнять извлечение значений вручную по всему коду
